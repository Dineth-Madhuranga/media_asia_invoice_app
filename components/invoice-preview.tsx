    ...
    >
      ...
      <div className="flex justify-between items-start mb-8">
        <div className="flex flex-col">
          <div className="bg-invoice-red text-white text-lg font-semibold px-4 py-1 mb-4 inline-block">INVOICE</div>
      ...
      {/* Items Table */}
      <table className="w-full border-collapse mb-8" style={{ backgroundColor: 'white' }}>
        <thead>
        ...
        <tbody style={{ backgroundColor: 'white' }}>
          {items.map((item) => (
            <tr key={item.id} className="bg-white border-b border-invoice-border-grey text-invoice-dark-grey text-sm" style={{ backgroundColor: 'white !important' }}>
              <td className="py-2 px-4 align-top" style={{ backgroundColor: 'white' }}>{item.description}</td>
              <td className="py-2 px-4 text-center align-top" style={{ backgroundColor: 'white' }}>
                {item.units} {item.unitType}
              </td>
              <td className="py-2 px-4 text-right align-top" style={{ backgroundColor: 'white' }}>{item.unitPrice.toFixed(2)}</td>
              <td className="py-2 px-4 text-right align-top" style={{ backgroundColor: 'white' }}>{(item.units * item.unitPrice).toFixed(2)}</td>
            </tr>
          ))}
          {discount > 0 && (
            <tr className="bg-white border-b border-invoice-border-grey text-invoice-dark-grey text-sm" style={{ backgroundColor: 'white !important' }}>
              <td className="py-2 px-4 align-top font-semibold" colSpan={3} style={{ backgroundColor: 'white' }}>
                Special Discount
              </td>
              <td className="py-2 px-4 text-right align-top" style={{ backgroundColor: 'white' }}>({discount.toFixed(2)})</td>
            </tr>
          )}
        </tbody>
        <tfoot>
      ...
      <div className="flex justify-end mt-12">
        {includeSignature ? (
          ...
          <div className="flex flex-col items-center text-invoice-dark-grey text-sm">
            <p className="italic text-xs">{"This is a computer generated invoice. No signature required.                                                                                                                                                                                                                                                                                                                                                                                                      "}</p>
          </div>
        )}
      </div>
